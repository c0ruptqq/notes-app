...
export function searchInJSON(searchTerm, options = { caseSensitive: false, partialMatch: false }) {
  const results = [];
  if (searchTerm !== "") {
    ...
    jsonData.forEach(item => {
      // Search in title
      if (item.title) {
        const titleMatches = searchAndReturnContext(item.title, item.route, searchTerm, options); // 5 words of context
      ...
      // Search in content
      const contentMatches = searchAndReturnContext(item.content, item.route, searchTerm, options); // 5 words of context
...
function searchAndReturnContext(text, route, searchTerm, options) {
